...
export default async (req) => {
  try {
    ...
    // 1. Query the record to get the attachment's current version URL
    ...
    // fileVal.url looks like: /files/{tableId}/{recordId}/{fieldId}/{version}
    // 2. Download the PDF (QB returns base64 in the response body)
    ...
    // 3. Render page 1 of the PDF to PNG
    // viewportScale: PDF native is 72 DPI, so scale = dpi / 72
    ...
    // 4. Trim blank margins. Flatten onto white first so transparent
    //    regions count as blank, then trim near-white edges.
    ...
    // 5. Upload the PNG back to the record
